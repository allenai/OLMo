...
do
    ...
    do
        ...
        bash $SCRIPT_DIR/run_subsetter.sh $out_dir c4_100_domains 10000 $split '(.*?)\.json\.gz\.(test|val)\.json\.gz' c4_100_domains_$subdomain 1 false $file
    done
done
...
bash $SCRIPT_DIR/run_subsetter.sh $out_dir pile 1000000 val false false $batch_size 'pile_set_name' $in_dir/pile/val/*.gz
bash $SCRIPT_DIR/run_subsetter.sh $out_dir pile 1000000 test false false $batch_size 'pile_set_name' $in_dir/pile/test/*.gz

bash $SCRIPT_DIR/run_subsetter.sh $out_dir m2d2_s2orc 1000000 val '^(.+)(?:_test|_valid)\.jsonl\.gz$' false $batch_size false $in_dir/m2d2/s2orc_paragraphs/val/*.gz
bash $SCRIPT_DIR/run_subsetter.sh $out_dir m2d2_s2orc 1000000 test '^(.+)(?:_test|_valid)\.jsonl\.gz$' false $batch_size false $in_dir/m2d2/s2orc_paragraphs/test/*.gz

bash $SCRIPT_DIR/run_subsetter.sh $out_dir m2d2_wiki 1000000 val '^(.+)(?:_test|_valid)\.jsonl\.gz$' false $batch_size false $in_dir/m2d2/wikipedia/val/*.gz
bash $SCRIPT_DIR/run_subsetter.sh $out_dir m2d2_wiki 1000000 test '^(.+)(?:_test|_valid)\.jsonl\.gz$' false $batch_size false $in_dir/m2d2/wikipedia/test/*.gz

bash $SCRIPT_DIR/run_subsetter.sh $out_dir ice 1000000 true '^(.+)\.jsonl\.gz$' false $batch_size false $in_dir/ice/*.gz
