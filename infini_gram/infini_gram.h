...
public:
    ...
    NGramLanguageModeling(const string index_dir, const TokenId eos_token_id)
        ...
        for (auto s = 0; s < _num_shards; s++) {
            ...
            assert (ds_size % sizeof(TokenId) == 0);
            U64 tok_cnt = ds_size / sizeof(TokenId);
    ...
    virtual FindResult find(const vector<TokenId> &input_ids, const size_t start, const size_t end, vector<pair<U64, U64>> hint_segment_by_shard = {}) const {
    ...
    virtual DistResult ntd(const vector<TokenId> &input_ids, const size_t start, const size_t end, const size_t support, const FindResult &prompt_find_result) const {
        ...
        for (U64 i = 0; i < support; i++) {
            ...
            TokenId token_id = _convert_offset_to_token_id(shard, offset);
            tokens[i] = token_id;
        }
    ...
    virtual void find_5gram_dense(const vector<TokenId> input_ids, vector<FindResult>* results) const {
    ...
    virtual vector<vector<DistResult>> ntd_dense_batch(const vector<vector<TokenId>> &input_idss, const U64 method, const U64 min_cnt, const size_t support, const bool debug = false, vector<vector<TokenId>>* lfnss = nullptr) const {
...
public:
    ...
    inline TokenId _convert_offset_to_token_id(const DatastoreShard &shard, const U64 offset) const {
        ...
        TokenId token_id; // no need to initialize
        memcpy(&token_id, shard.ds + offset, sizeof(TokenId));
        // If you see \xff\xff, this actually means we're at the very end of a document.
        if (token_id == (TokenId)(-1)) token_id = _eos_token_id;
        return token_id;
    }
