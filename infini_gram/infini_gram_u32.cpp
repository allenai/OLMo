...
void rank_processor(const size_t rank, const U64 MAX_BATCH_SIZE, const U64 MAX_SEQ_LEN, const U64 MAX_SUPPORT, const string MODE, const shared_ptr<NGramLanguageModeling> lm) {
    ...
    vector<U32> input_ids(MAX_BATCH_SIZE * MAX_SEQ_LEN);
    vector<U32> output_ids(MAX_BATCH_SIZE * MAX_SEQ_LEN * MAX_SUPPORT);
    ...
    while (true) {
        ...
        fifo_query.read(reinterpret_cast<char*>(input_ids.data()), batch_size * seq_len * sizeof(U32));
        if (MODE == "debug") {
        ...
        vector<vector<U32>> input_idss(batch_size, vector<U32>(seq_len));
        ...
        fifo_response.write(reinterpret_cast<const char*>(output_ids.data()), batch_size * seq_len * support * sizeof(U32));
        fifo_response.flush();
