...
for batch_size in [1024, 512, 256, 128, 64, 32, 16, 8, 4, 2, 1]:
    ...
    for it in range(110):
        ...
        input_buf = ds[tok_offset * 4:(tok_offset + batch_size * max_seq_len) * 4]
        input_idss = np.frombuffer(input_buf, dtype=np.uint8).view(np.uint32).astype(np.int64).reshape(batch_size, max_seq_len)
        input_idss = torch.tensor(input_idss, dtype=torch.int64)
        input_idss[input_idss == 2**32-1] = 100257
        if 'v5' in index_dir:
