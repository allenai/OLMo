...
with open('peteish1/peteish1-b384-lr5.2e-4.csv') as f:
    reader = csv.DictReader(f)
    data2 = list(reader)
...
plt.plot([float(x['throughput/total_tokens']) for x in data2], [float(x['eval/downstream/hellaswag_len_norm']) for x in data2], label='peteish1-b384-lr5.2e-4')
# save plot
plt.xlabel('total_tokens')
plt.ylabel('hellaswag_len_norm')
plt.legend()
plt.savefig('peteish1/peteish1-b384-lr5.2e-4.png', dpi=300, bbox_inches='tight')
