...
gantry run \
  ...
  --venv base \
  --yes \
  -- /bin/bash -c "torchrun --nnodes ${BEAKER_NODES}:${BEAKER_NODES} --nproc-per-node ${BEAKER_GPUS} ${TORCHRUN_CONFIG} scripts/train.py ${CONFIG_PATH} --run_name=${FULL_RUN_NAME} --wandb.group=${RUN_NAME} ${LOAD_PATH_ARG} ${EXTRA_ARGS}"
