...
MODEL_CONFIGS = {
    "150M": MODEL_CONFIG_150M.update_with(mup_base_shapes=BASE_SHAPES),
    "300M": MODEL_CONFIG_150M.update_with(d_model=704, mup_base_shapes=BASE_SHAPES),
    "750M": MODEL_CONFIG_150M.update_with(d_model=1152, mup_base_shapes=BASE_SHAPES),
    "1B": MODEL_CONFIG_150M.update_with(d_model=1344, mup_base_shapes=BASE_SHAPES),
    "7B": MODEL_CONFIG_150M.update_with(
        d_model=3648,
        init_device="meta",
        mup_base_shapes=BASE_SHAPES,
    ),
}
...
def config_from_args(args: argparse.Namespace) -> TrainConfig:
    ...
    remote_save_folder = f"s3://ai2-llm/checkpoints/OLMo-mup-ladder/{run_name}"
    ...
    # From the muTransfer paper:
    # "For a sufficiently large batch size, the optimal learning rate is roughly constant."
    ...
    return TrainConfig(
        run_name=run_name,
        seed=6198,
        wandb=None if not args.wandb else WandbConfig(name=run_name, group=run_name, project="olmo-mup-ladder"),
        model=model_config,
...
if __name__ == "__main__":
    ...
    for subparser in [dump_parser, train_parser]:
        ...
        subparser.add_argument("--lr", type=float, required=False, default=1e-3)
