...
gantry run \
  ...
  --shared-memory 10GiB \
  --yes \
  -- /bin/bash -c "scripts/beaker/mup/mup_peteish1-wsd-augusta.sh \$BEAKER_LEADER_REPLICA_HOSTNAME ${NUM_NODES} \$BEAKER_REPLICA_RANK ${SIZE} ${D_MODEL} ${LR} ${LOAD_STEP} ${ANNEAL_STEPS} ${@}"