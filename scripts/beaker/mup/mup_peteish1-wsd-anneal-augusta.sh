...
# Setting some of these at gantry level
...
pip install --no-deps git+https://github.com/2015aroras/mup@00993cbb38449c86ebe404ef1fe0172da6ceec35
pip freeze
...
BASE_RUN_NAME=$NAME-$(date -u +"%Y%m%d_%H%M%S")
SAVE_FOLDER=/data/$BASE_RUN_NAME
mkdir -p $SAVE_FOLDER
...
export BASE_RUN_NAME="peteish1_wsd_mup${SIZE}_${LR}"
export GROUP_NAME="peteish1_wsd_mup${SIZE}"

export GOOGLE_CLOUD_PROJECT=ai2-allennlp

ANNEAL_NAME="${BASE_RUN_NAME}_anneal${LOAD_STEP}_${ANNEAL_STEPS}"
SAVE_FOLDER=/data/$ANNEAL_NAME
mkdir -p $SAVE_FOLDER
...
torchrun \
  ...
  scripts/train.py configs/mup/peteish1-augusta.yaml \
    --run_name=$ANNEAL_NAME \
    --wandb.name=$ANNEAL_NAME \
