...
gantry run \
  ...
  --venv base \
  --yes \
  -- /bin/bash -c "sudo apt-get update; sudo apt-get -y install dnsutils; source scripts/beaker/warm_hf_cache.sh && torchrun --nnodes ${NUM_NODES}:${NUM_NODES} --nproc-per-node 8 --rdzv_id=101 --rdzv_backend=c10d --rdzv_endpoint=\$(dig -4 +short \${BEAKER_LEADER_REPLICA_HOSTNAME}):29400 scripts/train.py ${CONFIG_PATH} ${ARGS}"
