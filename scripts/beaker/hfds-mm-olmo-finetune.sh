...
# if so, use that instead of configs/llava/instruct_tune/hfds-openai-vit-l-14-336-mlp2x_gelu-olmo-7b-instruct-hf-seq2048.yaml
if [ -z ${CONFIG_PATH+x} ]; then
  export CONFIG_PATH=configs/llava/instruct_tune/hfds-openai-vit-l-14-336-mlp2x_gelu-olmo-7b-instruct-hf-seq2048.yaml
else
...
gantry run \
  ...
  ${WANDB_API_KEY_ARG} \
  --venv base \
  -- /bin/bash -c "DATA_DIR=${DATA_DIR} OUTPUT_DIR=${OUTPUT_DIR} WANDB_ENTITY=${WANDB_ENTITY} CONFIG_PATH=${CONFIG_PATH} bash scripts/llava/finetune.sh"
