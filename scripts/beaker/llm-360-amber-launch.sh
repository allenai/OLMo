...
gantry run \
  ...
  --task-name llm-360-amber-data-repro-match-dirk-baseline-normal-init \
  --description "OLMO 7B LLM 360 Amber data with congif matching Dirks baseline with Amberish init" \
  --priority high \
  --preemptible \
