...
gantry run \
  ...
  --task-name amberish1-wd-all \
  --description "Amberish 1B train with WD everywhere" \
  --priority urgent \
  --preemptible \
  ...
  --yes \
  --timeout=-1 \
  -- /bin/bash -c "scripts/beaker/amberish/amberish1-wd-all.sh \$BEAKER_LEADER_REPLICA_HOSTNAME ${NUM_NODES} \$BEAKER_REPLICA_RANK"
