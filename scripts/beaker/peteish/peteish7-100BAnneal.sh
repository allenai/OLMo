...
torchrun \
  ...
  scripts/train.py \
    configs/peteish7-weka.yaml \
      ...
      --optimizer.learning_rate=0.000061351 \
      --scheduler.name=linear_with_warmup \
      --scheduler.units=steps \
      --scheduler.t_warmup=928646 \
      --scheduler.t_max=$((928646 + MAX_STEPS)) \
      --max_duration=$((928646 + MAX_STEPS)) \
      --stop_at=$((928646 + MAX_STEPS + 10)) \
