...
gantry run \
  ...
  --task-name peteish1-normbefore-jupiter \
  --description "Pete-ish 1B, norm before, run on jupiter" \
  --priority high \
  ...
  -- /bin/bash -c "\
    ...
    torchrun \
        ...
        scripts/train.py \
            ...
            --run_name="peteish1-normbefore-jupiter" \
            --save_interval_ephemeral=null \
            --save_overwrite \
            --load_path=\\\${path.last_checkpoint:\\\${save_folder}} \
            --model.norm_after=false --scheduler.t_max=5e12 \
    "
