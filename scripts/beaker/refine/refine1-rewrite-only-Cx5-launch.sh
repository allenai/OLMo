...
gantry run \
  ...
  --allow-dirty \
  --task-name refine1-rewrite-unfiltered-cx5-20240822 \
  ...
  --yes \
  --timeout=-1 \
  -- /bin/bash -c "scripts/beaker/refine/refine1-rewrite-only-Cx5.sh \$BEAKER_LEADER_REPLICA_HOSTNAME ${NUM_NODES} \$BEAKER_REPLICA_RANK"
