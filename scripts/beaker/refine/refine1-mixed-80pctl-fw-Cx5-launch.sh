...
gantry run \
  ...
  --allow-dirty \
  --task-name refine1-mixed-80pctl-fw-cx5-20240924 \
  ...
  --yes \
  --timeout=-1 \
  -- /bin/bash -c "scripts/beaker/refine/refine1-mixed-80pctl-fw-Cx5.sh \$BEAKER_LEADER_REPLICA_HOSTNAME ${NUM_NODES} \$BEAKER_REPLICA_RANK"
