...
gantry run \
  ...
  --allow-dirty \
  --task-name refine1-mixed-bottom-20pctl-fw-cx5-20241008 \
  ...
  --yes \
  --timeout=-1 \
  -- /bin/bash -c "scripts/beaker/refine/refine1-mixed-bottom-20pctl-fw-Cx5.sh \$BEAKER_LEADER_REPLICA_HOSTNAME ${NUM_NODES} \$BEAKER_REPLICA_RANK"
