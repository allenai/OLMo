...
gantry run \
  ...
  --allow-dirty \
  --task-name refine1-rewrites-filtered \
  ...
  --yes \
  --timeout=-1 \
  -- /bin/bash -c "scripts/beaker/refine/refine1-rewrite-only-filtered-Cx2.sh \$BEAKER_LEADER_REPLICA_HOSTNAME ${NUM_NODES} \$BEAKER_REPLICA_RANK"
