...
def dump_run(run, output_file: Path):
    ...
    if output_file.is_file():
        log.info("Reading existing info for %s from %s", run.path[-1], output_file)
        ...
        log.info("Found %d entries in %s", len(data), output_file)
    else:
        f = output_file.open("x", encoding="UTF-8")

    def write_out_results():
        log.info("Writing %d entries to %s", len(data), output_file)
        ...
        for step, values in ordered_data:
