...
def build_olmo_model(base_olmo_model: Optional[Any], device, use_fsdp=False):
    ...
    parameters_to_read = {name for name, _ in base_olmo_model.named_parameters()}

    if base_olmo_model is not None:
        with torch.no_grad():
            # embeddings
            assert olmo_model.transformer.wte.weight.dtype == base_olmo_model.model.embed_tokens.weight.dtype
            assert olmo_model.transformer.wte.weight.shape == base_olmo_model.model.embed_tokens.weight.shape
            olmo_model.transformer.wte.weight.copy_(base_olmo_model.model.embed_tokens.weight)
            parameters_to_set.remove("transformer.wte.weight")
            parameters_to_read.remove("model.embed_tokens.weight")

            # output projection
            assert base_olmo_model.lm_head.weight.shape == olmo_model.transformer.ff_out.weight.shape
            assert base_olmo_model.lm_head.weight.dtype == olmo_model.transformer.ff_out.weight.dtype
            olmo_model.transformer.ff_out.weight.copy_(base_olmo_model.lm_head.weight)
            parameters_to_set.remove("transformer.ff_out.weight")
            parameters_to_read.remove("lm_head.weight")

            # final layer norm
            assert base_olmo_model.model.norm.weight.shape == olmo_model.transformer.ln_f.weight.shape
            assert base_olmo_model.model.norm.weight.dtype == olmo_model.transformer.ln_f.weight.dtype
            olmo_model.transformer.ln_f.weight.copy_(base_olmo_model.model.norm.weight)
            ...
            for i, (base_olmo_layer, olmo_layer) in enumerate(zip(base_olmo_model.model.layers, olmo_model.transformer.blocks)):
                ...
                olmo_layer.attn_norm.weight.copy_(base_olmo_layer.input_layernorm.weight)
                ...
                assert base_olmo_layer.post_attention_layernorm.weight.shape == olmo_layer.ff_norm.weight.shape
                assert base_olmo_layer.post_attention_layernorm.weight.dtype == olmo_layer.ff_norm.weight.dtype
                olmo_layer.ff_norm.weight.copy_(base_olmo_layer.post_attention_layernorm.weight)
                ...
                # torch.allclose().
                assert base_olmo_layer.self_attn.q_proj.weight.dtype == olmo_layer.att_proj.weight.dtype
                assert base_olmo_layer.self_attn.k_proj.weight.dtype == olmo_layer.att_proj.weight.dtype
                assert base_olmo_layer.self_attn.v_proj.weight.dtype == olmo_layer.att_proj.weight.dtype
                new_att_proj = torch.cat(
                    [
                        base_olmo_layer.self_attn.q_proj.weight,
                        base_olmo_layer.self_attn.k_proj.weight,
                        base_olmo_layer.self_attn.v_proj.weight,
                    ],
                    dim=0,
                )
                ...
                # attention out
                assert base_olmo_layer.self_attn.o_proj.weight.shape == olmo_layer.attn_out.weight.shape
                assert base_olmo_layer.self_attn.o_proj.weight.dtype == olmo_layer.attn_out.weight.dtype
                olmo_layer.attn_out.weight.copy_(base_olmo_layer.self_attn.o_proj.weight)
                ...
                assert base_olmo_layer.mlp.down_proj.weight.shape == olmo_layer.ff_out.weight.shape
                assert base_olmo_layer.mlp.down_proj.weight.dtype == olmo_layer.ff_out.weight.dtype
                olmo_layer.ff_out.weight.copy_(base_olmo_layer.mlp.down_proj.weight)
...
base_olmo_model = build_olmo_model(None, base_olmo_device)
olmo_model = build_olmo_model(base_olmo_model, olmo_device, use_fsdp=use_fsdp)
...
# run on base olmo
torch.manual_seed(42)
base_olmo_output = base_olmo_model(test_batch.to(device=non_meta_device(base_olmo_device)))
base_olmo_logits = base_olmo_output.logits
log.info(f"Base OLmo logits: {base_olmo_logits}")
...
torch.manual_seed(42)
test_all_approx_close(base_olmo_logits.cpu().float(), olmo_logits.cpu().float(), atol=1e-4, rtol=1e-3, count=10)
if not torch.allclose(base_olmo_logits.cpu().float(), olmo_logits.cpu().float(), atol=1e-4, rtol=1e-3):
...
if use_fsdp:
    ...
    olmo_generation_model = build_olmo_model(base_olmo_model, device="cpu", use_fsdp=False)

log.info(f"OLMo generation: {generate(olmo_generation_model, tokenizer, test_string)}")
log.info(f"Base OLMo generation: {generate(base_olmo_model, tokenizer, test_string)}")
