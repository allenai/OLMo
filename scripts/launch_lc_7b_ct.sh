...
gantry run \
  ...
  --yes \
  --timeout=-1 \
  -- /bin/bash -c "NCCL_DEBUG=INFO NCCL_SOCKET_IFNAME=ib NCCL_IB_HCA=\"^=mlx5_bond_0\" scripts/beaker/lc_7b.sh \$BEAKER_LEADER_REPLICA_HOSTNAME ${NUM_NODES} \$BEAKER_REPLICA_RANK"
