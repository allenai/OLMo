...
def write_model(model_path, input_base_path, tokenizer_path=None, safe_serialization=True, fix_eos_token_id=True):
    ...
    olmo_config = OLMoConfig.from_pretrained(input_base_path)
    ...
    vocab_size = olmo_config.embedding_size if olmo_config.embedding_size is not None else olmo_config.vocab_size
    ...
    loaded = OLMoForCausalLM.from_pretrained(input_base_path).state_dict()
    ...
    if olmo_config.mlp_hidden_size is not None:
