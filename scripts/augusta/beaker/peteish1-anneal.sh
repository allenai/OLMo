...
pip install '.[train]'
# wandb 0.19.1 breaks `group_name_from_wandb.py`
pip install --upgrade wandb==0.19.0
pip freeze
...
STEPS_TO_TRAIN=$(python -c "print($LENGTH + $START_STEP)")
...
torchrun \
  ...
  scripts/train.py \
    configs/peteish1-google.yaml \
      ...
      --remote_save_folder="gs://ai2-llm/shanea/checkpoints/OLMo-small/$NAME/" \
      --try_load_latest_save \
      --save_overwrite \
      --load_path=gs://ai2-llm/shanea/checkpoints/OLMo-medium/$(python ./scripts/group_name_from_wandb.py $ORIGINAL_WANDB_RUN_ID)/step$START_STEP \
