...
gantry run \
  --workspace ai2/13B \
  --task-name $NAME \
  --description "Peteish7 annealing : $NAME with seed $SEED" \
  --priority high \
  --preemptible \
  ...
  --timeout=-1 \
  --retries 10 \
  -- /bin/bash -c "scripts/augusta/peteish7-seed-anneal.sh \$BEAKER_LEADER_REPLICA_HOSTNAME \$BEAKER_REPLICA_RANK $SEED"
