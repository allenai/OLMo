...
gantry run \
  ...
  --task-name peteish1-wd0.033-augusta \
  --description "Peteish1, weight decay 0.033, run on augusta" \
  --priority high \
  --preemptible \
  ...
  --allow-dirty \
  --retries 10 \
  -- /bin/bash -c "scripts/augusta/peteish1-wd0.033.sh \$BEAKER_LEADER_REPLICA_HOSTNAME \$BEAKER_REPLICA_RANK"
