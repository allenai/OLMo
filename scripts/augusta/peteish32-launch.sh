...
gantry run \
  --workspace ai2/13B \
  --task-name peteish32-oldtrainer \
  --description "Peteish32 with old trainer" \
  --priority urgent \
  --preemptible \
  ...
  --allow-dirty \
  --retries=10 \
  -- /bin/bash -c "scripts/augusta/peteish32.sh \$BEAKER_LEADER_REPLICA_HOSTNAME \$BEAKER_REPLICA_RANK"
