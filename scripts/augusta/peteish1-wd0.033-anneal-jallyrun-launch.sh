...
gantry run \
  ...
  --task-name peteish1-wd0.033-augusta-anneal-jallyrun \
  --description "Peteish1, weight decay 0.033, run on augusta, anneal from 1907369 with jallyrun data" \
  --priority high \
  --preemptible \
  ...
  --allow-dirty \
  --retries 10 \
  -- /bin/bash -c "scripts/augusta/peteish1-wd0.033-anneal-jallyrun.sh \$BEAKER_LEADER_REPLICA_HOSTNAME \$BEAKER_REPLICA_RANK"
