...
gantry run \
  --workspace ai2/13B \
  --task-name peteish7-2xbsz-fancy \
  --description "Peteish7 with twice the batch size and fancy adjustments" \
  --priority high \
  --preemptible \
  ...
  --allow-dirty \
  --retries 10 \
  -- /bin/bash -c "scripts/augusta/batchsize/peteish7-2xbsz-fancy.sh \$BEAKER_LEADER_REPLICA_HOSTNAME \$BEAKER_REPLICA_RANK"
