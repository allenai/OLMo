...
import pytest
from olmo.config import ModelConfig, TokenizerConfig, TrainConfig
...
@pytest.fixture(scope="module")
def lorem_ipsum() -> str:
    return TestFixtures.LOREM_IPSUM_1.replace("\n", " ").strip()

@pytest.fixture(scope="module")
def lorem_ipsum_docs() -> List[str]:
    return [text.replace("\n", " ").strip() for text in (TestFixtures.LOREM_IPSUM_1, TestFixtures.LOREM_IPSUM_2)]
