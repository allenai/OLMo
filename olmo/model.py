...
class ComplexRotaryEmbedding(RotaryEmbedding):
    """
    ...
    def get_rotary_embedding(self, seq_len: int, device: torch.device) -> torch.Tensor:
        ...
        with torch.autocast(device.type, enabled=False):
            dim = self.config.d_model // self.config.n_heads
            inv_freq = 1.0 / (
                500_000 ** (torch.arange(0, dim, 2, device=device, dtype=torch.float)[: (dim // 2)] / dim)
            )
            seq = torch.arange(seq_len, device=device, dtype=torch.float)
            freqs = einsum("i , j -> i j", seq, inv_freq)
...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if self.config.rope:
            self.rotary_emb = (
                ComplexRotaryEmbedding(config, self.__cache)
                if self.config.complex_rope
                else RotaryEmbedding(config, self.__cache)
            )
