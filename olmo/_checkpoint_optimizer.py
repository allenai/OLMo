...
def _create_colwise_spec(
    ...
    if pg is None:
        placements = [f"rank:{idx}/{_gen_rank_device(idx)}" for idx in range(dist.get_world_size())]
    else:
        placements = [f"rank:{idx}/{_gen_rank_device(dist.get_global_rank(pg, idx))}" for idx in range(pg.size())]
...
class _ReaderWithOffset(DefaultLoadPlanner):
    ...
    def create_local_plan(self) -> LoadPlan:
        ...
        for fqn, obj in self.state_dict.items():
            ...
            shard_md.shard_offsets = _element_wise_add(shard_md.shard_offsets, offset)
            local_shards = [Shard(original_shard.tensor, shard_md)]

            reqs = _create_sharded_read_items(fqn, cast(TensorStorageMetadata, md), local_shards)
            ...
            for wi in reqs:
                ...
                original_offset = _element_wise_sub(wi.dest_index.offset, offset)
...
def load_sharded_optimizer_state_dict(
    ...
    if dp_pg is None:
        sharding_spec = ChunkShardingSpec(
            dim=0,
            placements=[f"rank:{i}/cuda:{i % torch.cuda.device_count()}" for i in range(dist.get_world_size())],
        )
    else:
