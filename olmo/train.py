...
class Trainer:
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        # Setup the hook for accumulating the squared grad norm 
        assert self.cfg.device_train_grad_accum is not None
        devices_instance_squared_norms = []

        def save_grad_squared_norm(grad: torch.Tensor) -> None:
            nonlocal devices_instance_squared_norms
            devices_instance_squared_norms.append(torch.pow(grad.detach(), 2).sum())

        backwards_hook = save_grad_squared_norm if should_log_optim_metrics_this_step else None
        ...
        if should_log_optim_metrics_this_step:
            ...
            # Collect sum of instance grad norms
            instances_squared_norms = torch.cat(devices_instance_squared_norms).to(self.device)
            dist.reduce(instances_squared_norms, 0, op=dist.ReduceOp.SUM, group=self.fsdp_model.process_group)
            instances_squared_norms.sqrt_()
            metrics["optim/instances_grad_norm_sum"] = instances_squared_norms.sum().item()
