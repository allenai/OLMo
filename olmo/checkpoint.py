...
class TorchLegacyShardedCheckpointer(Checkpointer):
    ...
    def _unshard_using_sharded_mem(
        self, shard: Any, world_size: int, device: torch.device, shard_dir: PathOrStr
    ) -> Any:
        return self._unshard_state_using_shared_mem(shard, world_size, device, (str(shard_dir).replace("/", "_"),))

    def _unshard_state_using_shared_mem(
        self, shard: Any, world_size: int, device: torch.device, key: Tuple
    ) -> Any:
        if isinstance(shard, (list, tuple, set)):
            return shard.__class__(
                self._unshard_state_using_shared_mem(sub_shard, world_size, device, key + (i,))
                for i, sub_shard in enumerate(shard)
            )
        elif isinstance(shard, dict):
            return {
                name: self._unshard_state_using_shared_mem(shard[name], world_size, device, key + (name,))
                for name in shard.keys()
            }
        elif isinstance(shard, ShardedTensor):
            return self._unshard_tensor_using_shared_mem(shard, world_size, device, key)
    ...
    def _unshard(self, input_dir: PathOrStr, device: torch.device, skip_keys: Optional[Set[str]] = None):
        ...
        log.info("Loading a shard on the main process")
        with self._patch_sharded_tensor_load():
            base_shard = torch.load(shard_filepaths[0], map_location="cpu")

        for key in skip_keys:
            if key in base_shard:
                del base_shard[key]

        log.info("Unsharding from %d shards ...", world_size)
        return self._unshard_using_sharded_mem(base_shard, world_size, device, input_dir)
