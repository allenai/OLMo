...
class ExampleReader:
  ...
  def _build_sequence(
    ...
    # TODO possibly this code and truncation logic is better off somewhere else
    ...
    for segment_id, (file_id, start_byte, num_bytes) in enumerate(examples):
      ...
      for example in examples:
        if sequence_length:
          ...
          # Possibly truncate this example, we remove chunks until we have a non-masked chunk
          # that has at least some tokens that do not need to get truncated
          # This means we could truncate the document and then add in a new document after it
          while example and (
