...
def build_sa(args):
    ...
    # Make sure that parts contain whole tokens (DSIZE bytes)
    if S % DSIZE != 0:
        S += DSIZE - S % DSIZE
    ...
    for batch_start in tqdm(list(range(0, total_jobs, parallel_jobs))):
        ...
        for (s, e) in batch_ranges:
            cmd = f'./suffix_array_u32 make-part --data-file {ds_path} --parts-dir {parts_dir} --start-byte {s} --end-byte {e}'
    ...
    cmd = f'./suffix_array_u32 merge --merged-dir {merged_dir} --suffix-path {" --suffix-path ".join(files)} --num-threads {args.cpus} --hacksize {HACK}'
    pipe = os.popen(cmd)
    output = pipe.read()
    ...
    if table_size % (tok_size // DSIZE) != 0:
