...
def tokenize(args):
    ...
    eos_seq = bytearray(np.array([50279], dtype=np.uint16).view(np.uint8).tobytes())
    print(f'eos_seq = {eos_seq}')
    ...
    for data_path in tqdm(data_paths):
        ...
        if byte_arr[-2:] == eos_seq:
            byte_arr = byte_arr[:-2]
        byte_arr = eos_seq + byte_arr
        i = 0
        while True:
            i = byte_arr.find(eos_seq, i)
            if i == -1:
                break
            if i % 2 == 1:
                i += 1
                continue
            byte_arr[i:i+2] = args.doc_sep
            ods.append(od + i)
            i += 2
